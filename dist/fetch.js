...
async function fetchPRTitles(prNumbers) {
    const prTitles = new Map();
    console.log(`Starting to fetch titles for ${prNumbers.length} PRs:`, prNumbers);
    ...
    console.log(`Successfully fetched ${prTitles.size} of ${prNumbers.length} PR titles`);
    console.log('Fetched PR titles:', Array.from(prTitles.entries()));
    return prTitles;
}
// Function to parse XML run data into structured format
function parseRunData(xmlText) {
...
function parseRunDetails(xmlText, runNumber) {
    const parser = new DOMParser();
    const xmlDoc = parser.parseFromString(xmlText, "text/xml");
    const blobs = xmlDoc.getElementsByTagName("Blob");
    ...
    for (const [testFolderPath, folder] of testFolders) {
        ...
        if (pathParts.length >= 2) {
            ...
            if (cleanPathParts.length >= 2) {
                ...
                const status = folder.hasPassed ? 'passed' : 'failed';
...
export async function fetchRunDetails(runNumber) {
    try {
...
export async function fetchRunData() {
    try {
        ...
        if (prNumbers.length > 0) {
            console.log(`Fetching titles for ${prNumbers.length} PRs...`);
